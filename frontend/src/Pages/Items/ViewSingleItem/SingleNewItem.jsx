...
import { getElectronics,getCleaning,getStationary, getUser } from '../../../Components/redux/store';
export default function SingleNewItem({el,PendingHandler}) {
  ...
  const updateHandler=async function()
  {
    ...
    const res=await fetch(`/api/item/approvenewitem/${el._id}`,{
        ...
        body:JSON.stringify({status:status,itemcode:[electronics.length,stationary.length,cleaning.length]})

...
const UserForm = () => {
  ...
  const [requisitionNumber,setrequisitionNumber]=useState(null);
  const [formData, setFormData] = useState({
    ...
    requisition: false,
    requisitionNumber: '', // Initialize it with an empty string first
  ...
  useEffect(() => {
    async function viewAllIndent() {
      ...
      if (data.allIndent) {
        ...
        const year = data.allIndent[fetchedIndentNumber - 1]?.requisitionNumber?.slice(0, 8);
   
        const number = data.allIndent[fetchedIndentNumber - 1]?.requisitionNumber?.slice(8);
        ...
        let yearString="";
               ...
               if(currentMonth >= 3 && currentMonth <= 11)
               {
                ...
                yearString= `${currentYear - 1}${currentYear}`;
               }

         
   
        const updatedRequisitionNumber = (yearString === year) ?
          `${yearString}${(+number) + 1}` :
          `${yearString}${1}`;

        setrequisitionNumber(updatedRequisitionNumber);
        setFormData(prevData => ({ ...prevData, requisitionNumber: updatedRequisitionNumber }));
      }
    }
    viewAllIndent();
  }, [requisitionNumber]);
  ...
  const handleSubmitSuccess = () => {
    ...
    setTimeout(() => {
      ...
      setFormData({
        ...
        requisition:false,
        requisitionNumber: requisitionNumber, // Keep the requisition number unchanged
        itemArray:[InitialItem],
        budget: 0,
        amount:0,
        comment: "",
      });
      setrequisitionNumber(requisitionNumber); // Update requisitionNumber state with the fetched value
    }, 3000);
  };
  ...
  console.log(formData,requisitionNumber)
  return (
    ...
    <form className={`${styles.form} container`} onSubmit={handleSubmit}>
       <table  >
                <tr className={styles.IndenterDetails} >
                 ...
                 <input type="text" value={requisitionNumber} onChange={() => setrequisitionNumber(requisitionNumber)} />
